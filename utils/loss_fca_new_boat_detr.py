...
from utils.general import bbox_iou,xyxy2xywhn
from utils.torch_utils import is_parallel
...
class ComputeLoss_detr:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLoss_detr, self).__init__()
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        if p is not None:
            ...
            b=torch.Tensor(80)
            d=torch.max(p_clone[:,:,5])
            for i in range(80):
                b[i]=torch.max(p_clone[:,:,5+i])
            out=torch.max(b)
            out_index=torch.argmax(b)
            p_clone_clone[:,:,5:]= p_clone[ :,:,5:]#*iou
            loss=torch.max(p_clone_clone[:,:,14])
            #loss=torch.max(p_clone_clone[:,:,7])
            print('loss_Detr:',loss)
