...
def create_dataloader(path, imgsz, batch_size, stride,faces, texture_size, vertices, opt, hyp=None, augment=False, cache=False,image_render_out=640, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, faces, texture_size, vertices,imgsz, batch_size,
                                      ...
                                      prefix=prefix, mask_dir=mask_dir, ret_mask=ret_mask, phase=phase,image_render_out=image_render_out,model=model)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=640, batch_size=16, augment=False, hyp=None,image_render_out=640, rect=False, image_weights=False,
        ...
        self.mask_renderer = nmr.NeuralRenderer(img_size=image_render_out).to(self.device)
    ...
    def __getitem__(self, index):
        ...
        #或者尝试bilinear
        if imgs_pred.shape[2]!=640:
            imgs_pred = F.interpolate(imgs_pred, size=(640, 640), mode='bicubic', align_corners=False)
        # imgs_pred=
        # print(f"imgs_pred.shape{imgs_pred.shape}")
