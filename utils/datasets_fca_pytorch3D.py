...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        imgs_pred = self.renderer(self.mesh, cameras=cameras) 
        # eye=[(x+a/scale)*scale,(z+b/scale)*scale,(y+c/scale)*scale]
        # print(f"eye_beforetransform:{eye}")
        # #transform
        # if math.sqrt(eye[0] ** 2 + eye[2] ** 2) == 0:
        #     cosfi = 0
        #     sinfi = 0
        # else:
        #     cosfi = eye[0] / math.sqrt(eye[0] ** 2 + eye[2] ** 2)
        #     sinfi = eye[2] / math.sqrt(eye[0] ** 2 + eye[2] ** 2)
        # cossum = cosfi * math.cos(math.radians(veh_trans[1][1])) + sinfi * math.sin(math.radians(veh_trans[1][1]))
        # sinsum = math.cos(math.radians(veh_trans[1][1])) * sinfi - math.sin(math.radians(veh_trans[1][1])) * cosfi
        # eye=[math.sqrt(eye[0] ** 2 + eye[2] ** 2) * cossum, eye[1],math.sqrt(eye[0] ** 2 + eye[2] ** 2) * sinsum]
        # eye=torch.tensor(eye,dtype=torch.float32).to(self.device).unsqueeze(0)
        # print(f"eye_aftertransform:{eye}")
        #at=((0,0,0.1))
        #double to float
        ...
        # print(f"R:{R}")
        # print(f"T:{T}")
        # R, T = look_at_view_transform(eye=eye,at=((a,b,c),))
