...
def create_dataloader(path, imgsz, device,batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='',mask_dir='',ret_mask=False, phase='training',epoch=1):
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, device,imgsz, batch_size,
                                      ...
                                      prefix=prefix, mask_dir=mask_dir, ret_mask=ret_mask, phase=phase,epoch=epoch)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, device,img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',mask_dir='', ret_mask=False,phase='training',epoch=1):
    ...
    def __getitem__(self, index):
        ...
        # mask_renderer.renderer.renderer.camera_up = [0, 0, 1]
        # mask_renderer.renderer.renderer.background_color = [1, 1, 1]
        #一个魔术方法，定义了之后就可以直接class[index]获得想要的数据，也可以切片，
            ...
            length=math.sqrt(cam_trans[0][0]**2+
                                            ...
                                            cam_trans[0][2]**2)
            #camera parameters
            color_name=self.color_list[(index+self.color_list_add)%9]
            ...
            neural_img_dir=os.path.join(os.path.dirname(os.path.dirname(self.img_files[index])),'neural_renderer_result_pytorch3d')
            ...
            #再根据物体的片图结合之前输入的角度和距离信息就可以得到imgs_pred
